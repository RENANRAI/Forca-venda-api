...
public class EmpresasFiliaisErpService : IEmpresasFiliaisErpService
{
    ...
    private IReadOnlyList<EmpresasFiliaisErpDto> ParseResposta(string xml)
    {
        ...
        try
        {
            ...
            {
                var codEmp = e.Element("codEmp")?.Value?.Trim();
                var codFil = e.Element("codFil")?.Value?.Trim();

                if (string.IsNullOrWhiteSpace(codEmp) || string.IsNullOrWhiteSpace(codFil))
                    continue;
                ...
                lista.Add(new EmpresasFiliaisErpDto
                {
                    CodEmp = codEmp,
                    CodFil = codFil
                    /* NomeEmpresa = e.Element("nomEmp")?.Value,
                     NomeFilial = e.Element("nomFil")?.Value*/
                });
            }
        }
        catch (Exception ex)
